      ...
      <main className="min-h-screen bg-gray-50 py-8 sm:py-12">
        <div className="container mx-auto px-4">
          ...
          <div className="text-center mb-12">
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-[#179FCF] mb-4">Comprar ou Vender Milhas</h1>
              ...
              <div className="flex items-center space-x-4">
                {[1, 2, 3, 4].map((stepNumber) => (
                  <div key={stepNumber} className="flex items-center">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${step >= stepNumber ? "bg-[#179FCF] text-white" : "bg-gray-300 text-gray-600"}`}>
                      {step > stepNumber ? <Check size={20} /> : stepNumber}
                    </div>
                    {stepNumber < 4 && (<div className={`w-12 h-1 mx-2 ${step > stepNumber ? "bg-[#179FCF]" : "bg-gray-300"}`} />)}
                  </div>
                ))}
              </div>
            </div>
          ...
          {step === 1 && (
            <div className="max-w-2xl mx-auto">
              <div className="bg-white rounded-2xl shadow-lg p-8">
                <h2 className="text-2xl font-bold text-[#179FCF] mb-6 text-center">Selecione o Tipo, Companhia e Quantidade</h2>
                <div className="space-y-6">
                  <div>
                    <label className="block text-lg font-semibold text-gray-700 mb-3">Tipo de Operação</label>
                    <div className="grid grid-cols-2 gap-4">
                      <div onClick={() => setOperationType("comprar")} className={`p-4 border-2 rounded-lg cursor-pointer transition-colors text-center ${operationType === "comprar" ? "border-[#179FCF] bg-blue-50" : "border-gray-300 hover:border-[#179FCF]"}`}>
                      ...
                      <div onClick={() => setOperationType("vender")} className={`p-4 border-2 rounded-lg cursor-pointer transition-colors text-center ${operationType === "vender" ? "border-[#179FCF] bg-blue-50" : "border-gray-300 hover:border-[#179FCF]"}`}>
                  ...
                  <div>
                    ...
                    <select value={selectedAirline} onChange={(e) => setSelectedAirline(e.target.value)} className="w-full p-4 border-2 border-gray-300 rounded-lg focus:border-[#179FCF] focus:outline-none text-lg">
                  ...
                  <div>
                    ...
                    <select value={selectedMiles} onChange={(e) => setSelectedMiles(Number(e.target.value))} className="w-full p-4 border-2 border-gray-300 rounded-lg focus:border-[#179FCF] focus:outline-none text-lg" disabled={!selectedAirline}>
                      <option value="">Selecione a quantidade</option>
                      {milesOptions.map((miles) => (<option key={miles} value={miles}>{miles.toLocaleString()} milhas</option>))}
                    </select>
                  </div>
                  <button onClick={handleContinue} disabled={!operationType || !selectedAirline || !selectedMiles} className="w-full bg-[#179FCF] text-white py-4 px-6 rounded-lg font-semibold text-lg hover:bg-[#1489b8] transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed">Continuar</button>
                </div>
              </div>
            </div>
          )}
          ...
          {step === 2 && (
             <div className="max-w-2xl mx-auto"><div className="bg-white rounded-2xl shadow-lg p-8"><h2 className="text-2xl font-bold text-[#179FCF] mb-6 text-center">Tipo de Entrega</h2><div className="space-y-4"><div onClick={() => setDeliveryType("programado")} className={`p-6 border-2 rounded-lg cursor-pointer transition-colors ${deliveryType === "programado" ? "border-[#179FCF] bg-blue-50" : "border-gray-300 hover:border-[#179FCF]"}`}><div className="flex items-center justify-between"><div><h3 className="text-xl font-semibold text-gray-800">Programado</h3><p className="text-gray-600 mt-2">Entrega em até 72 horas úteis</p></div><div className={`w-6 h-6 rounded-full border-2 ${deliveryType === "programado" ? "border-[#179FCF] bg-[#179FCF]" : "border-gray-400"}`}>{deliveryType === "programado" && <Check size={16} className="text-white m-0.5" />}</div></div></div><div onClick={() => setDeliveryType("antecipado")} className={`p-6 border-2 rounded-lg cursor-pointer transition-colors ${deliveryType === "antecipado" ? "border-[#179FCF] bg-blue-50" : "border-gray-300 hover:border-[#179FCF]"}`}><div className="flex items-center justify-between"><div><h3 className="text-xl font-semibold text-gray-800">Antecipado</h3><p className="text-gray-600 mt-2">Entrega em até 24 horas</p><p className="text-red-600 text-sm mt-1 font-medium">⚠️ Pode incluir até 10% do valor em taxas</p></div><div className={`w-6 h-6 rounded-full border-2 ${deliveryType === "antecipado" ? "border-[#179FCF] bg-[#179FCF]" : "border-gray-400"}`}>{deliveryType === "antecipado" && <Check size={16} className="text-white m-0.5" />}</div></div></div><div className="flex gap-4"><button onClick={handleBack} className="flex-1 bg-gray-500 text-white py-4 px-6 rounded-lg font-semibold text-lg hover:bg-gray-600 transition-colors">Voltar</button><button onClick={handleContinue} disabled={!deliveryType} className="flex-1 bg-[#179FCF] text-white py-4 px-6 rounded-lg font-semibold text-lg hover:bg-[#1489b8] transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed">Continuar</button></div></div></div></div>
          )}

          {step === 3 && (
            <div className="max-w-2xl mx-auto"><div className="bg-white rounded-2xl shadow-lg p-8"><h2 className="text-2xl font-bold text-[#179FCF] mb-6 text-center">Confirme suas Informações</h2><div className="bg-gray-50 rounded-lg p-6 mb-6"><div className="space-y-4"><div className="flex justify-between"><span className="font-semibold text-gray-700">Operação:</span><span className="text-gray-900 capitalize">{operationType}</span></div><div className="flex justify-between"><span className="font-semibold text-gray-700">Companhia:</span><span className="text-gray-900">{selectedCompany?.name}</span></div><div className="flex justify-between"><span className="font-semibold text-gray-700">Quantidade:</span><span className="text-gray-900">{selectedMiles.toLocaleString()} milhas</span></div><div className="flex justify-between"><span className="font-semibold text-gray-700">Tipo de Entrega:</span><span className="text-gray-900 capitalize">{deliveryType}</span></div><div className="flex justify-between border-t pt-4 mt-4"><span className="font-bold text-gray-700">Valor Estimado:</span><span className="font-bold text-[#179FCF] text-lg">R$ {estimatedValue.toFixed(2)}</span></div>{deliveryType === "antecipado" && (<div className="bg-red-50 border border-red-200 rounded p-3 mt-4"><p className="text-red-700 text-sm"><strong>Atenção:</strong> A entrega antecipada pode incluir até 10% do valor em taxas.</p></div>)}</div></div><div className="flex gap-4"><button onClick={handleBack} className="flex-1 bg-gray-500 text-white py-4 px-6 rounded-lg font-semibold text-lg hover:bg-gray-600 transition-colors">Voltar</button><button onClick={handleContinue} className="flex-1 bg-[#179FCF] text-white py-4 px-6 rounded-lg font-semibold text-lg hover:bg-[#1489b8] transition-colors">Confirmar e Continuar</button></div></div></div>
          )}

          {step === 4 && (
            <div className="max-w-2xl mx-auto"><div className="bg-white rounded-2xl shadow-lg p-8"><h2 className="text-2xl font-bold text-[#179FCF] mb-6 text-center">Informações de Contato</h2><form onSubmit={handleSubmit} className="space-y-6"><div><label className="block text-lg font-semibold text-gray-700 mb-3">Nome Completo</label><input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className={`w-full p-4 border-2 rounded-lg focus:outline-none text-lg ${formErrors.name ? "border-red-500" : "border-gray-300 focus:border-[#179FCF]"}`} required disabled={isSubmitting} />{formErrors.name && <p className="text-red-500 text-sm mt-1">{formErrors.name}</p>}</div><div><label className="block text-lg font-semibold text-gray-700 mb-3">E-mail</label><input type="email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} className={`w-full p-4 border-2 rounded-lg focus:outline-none text-lg ${formErrors.email ? "border-red-500" : "border-gray-300 focus:border-[#179FCF]"}`} required disabled={isSubmitting} />{formErrors.email && <p className="text-red-500 text-sm mt-1">{formErrors.email}</p>}</div><div><label className="block text-lg font-semibold text-gray-700 mb-3">Telefone/WhatsApp</label><input type="tel" value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} className={`w-full p-4 border-2 rounded-lg focus:outline-none text-lg ${formErrors.phone ? "border-red-500" : "border-gray-300 focus:border-[#179FCF]"}`} required disabled={isSubmitting} />{formErrors.phone && <p className="text-red-500 text-sm mt-1">{formErrors.phone}</p>}</div><div><label className="block text-lg font-semibold text-gray-700 mb-3">CPF</label><input type="text" value={formData.cpf} onChange={(e) => setFormData({ ...formData, cpf: e.target.value })} className={`w-full p-4 border-2 rounded-lg focus:outline-none text-lg ${formErrors.cpf ? "border-red-500" : "border-gray-300 focus:border-[#179FCF]"}`} required disabled={isSubmitting} />{formErrors.cpf && <p className="text-red-500 text-sm mt-1">{formErrors.cpf}</p>}</div><div className="flex gap-4"><button type="button" onClick={handleBack} className="flex-1 bg-gray-500 text-white py-4 px-6 rounded-lg font-semibold text-lg hover:bg-gray-600 transition-colors" disabled={isSubmitting}>Voltar</button><button type="submit" className="flex-1 bg-[#179FCF] text-white py-4 px-6 rounded-lg font-semibold text-lg hover:bg-[#1489b8] transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed" disabled={isSubmitting}>{isSubmitting ? "Enviando..." : "Enviar Solicitação"}</button></div></form></div></div>
          )}
